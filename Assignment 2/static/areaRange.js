...
const chart2=(hourlyData)=>{
  ...
  let windSpeedDirectionData = hourlyData['intervals'].reduce((acc, ele, index) => {
        if (index % 2 == 0) {
            acc.push({
                x: new Date(ele.startTime).getTime(),
                value: ele.values.windSpeed,
                direction: ele.values.windDirection,
            });
        }
        return acc;
    }, []);
